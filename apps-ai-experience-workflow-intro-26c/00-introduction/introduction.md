# Introduction

## About this Workshop

Workflows are a predetermined set of tasks, which can be represented as a connected sequence of nodes. Each node performs a defined function, for example, extracting data, calling a business object, running an LLM, or sending an email. The node then passes its output to the next step. For business processes that require a strict order of execution, you can embed the required AI agents within the workflow.

Estimated Completion Time: 60 minutes

### **Objectives**

Learn about workflow agents & templates and execute one of the pre-built workflow template.  Below are node types available in AI Agent Studio that are used to build workflow agents.

![Image sizing applied](images/intro_image.png =70%x*)
By the end of this lab, participants will:
* Have a high-level understanding of how workflow agents are built.
* Have a quick peak into different types of nodes available to build workflow agents.
* Extend the workflow with a custom agent that you built that will check inventory for ordered items.
* Execute the workflow agent with human approval by uploading document in chat interface.

[](youtube:H-C5dH3w1Dg " ")


[Proceed to the first lab to get started!] (#next)

## Acknowledgements
* **Author** - [](var:author)
* **Contributors** - [](var:contributors)
* **Last Updated By/Date** - [](var:last_updated)